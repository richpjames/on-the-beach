...
export class ApiClient {
  ...
  async updateMusicItem(
    id: number,
    input: UpdateMusicItemInput,
  ): Promise<MusicItemFull | null> {
    const res = await fetch(`${this.baseUrl}/api/music-items/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(input),
  ...
  async getStacksForItem(musicItemId: number): Promise<Stack[]> {
    const res = await fetch(
      `${this.baseUrl}/api/stacks/items/${musicItemId}`,
    )
  ...
  async addItemToStack(
    ...
    const res = await fetch(
      `${this.baseUrl}/api/stacks/items/${musicItemId}/${stackId}`,
      { method: 'PUT' },
    )
    if (!res.ok) throw new Error(`addItemToStack failed: ${res.status}`)
  }

  async removeItemFromStack(
    musicItemId: number,
    stackId: number,
  ): Promise<void> {
    const res = await fetch(
