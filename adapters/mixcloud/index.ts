 ...
 * that in one place leaves the registry holding only the dispatch.
 */
import {
...
// ---------------------------------------------------------------------------
// Scraping a known Mixcloud URL
// ---------------------------------------------------------------------------

/**
 * Scrape a Mixcloud show page: oEmbed first — it names uploader and show, which
 * is everything a release needs — and only fetches the page when oEmbed came
 * back short.
 */
export async function scrapeMixcloud(
  url: string,
  timeoutMs: number,
): Promise<ScrapedMetadata | null> {
  const oEmbed = await fetchMixcloudOEmbed(url, timeoutMs);
  if (isCompleteMixcloudMetadata(oEmbed)) return oEmbed;

  const html = await fetchPageHtml(url, timeoutMs, { maxBytes: MAX_HEAD_BYTES, stopAt: "</head>" });
  if (html === null) return hasScrapedMetadata(oEmbed) ? oEmbed : null;

  return mergeMixcloudPageMetadata(html, parseOgTags(html), oEmbed);
}
