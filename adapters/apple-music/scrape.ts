...
/**
 * Scrape an Apple Music catalogue URL: Apple's JSON APIs first — no page fetch
 * at all when they come back complete — and the page's OG tags only to fill
 * what they missed.
 */
export async function scrapeAppleMusic(
  url: string,
  timeoutMs: number,
): Promise<ScrapedMetadata | null> {
  const api = await fetchAppleMusicApiMetadata(url, timeoutMs);
  if (isCompleteAppleMusicMetadata(api.merged)) return api.merged;

  const html = await fetchPageHtml(url, timeoutMs, {
    maxBytes: MAX_HEAD_BYTES,
    stopAt: "</head>",
  });
  if (html === null) return hasScrapedMetadata(api.merged) ? api.merged : null;

  return mergeAppleMusicPageMetadata(parseOgTags(html), api);
}
