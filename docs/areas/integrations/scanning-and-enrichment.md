...
- `server/soundcloud.ts` holds the SoundCloud playback knowledge: the resource urn (`soundcloud:tracks:123`) lifted from the hydra state in a page's body, and the widget URL built from it. SoundCloud's widget refuses permalinks and only plays api.soundcloud.com resources, so the urn is stored on the link at scrape time, like Bandcamp's `album_id`.
